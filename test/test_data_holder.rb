...
class DataHolder::Fix::Test < Minitest::Test
  ...
  def test_initialize
    ...
    assert_nil @holder.get(:edit_content)
  end


  def test_database_format
    # TODO: rewrite edit_content as actual data format(yaml)
    @holder.data_edit_content = 'test example'

    data = @holder.database_format
    assert_match $create_date_regexp, data.create_date
    assert_equal 1, data.fix_diary_id
    assert_equal 'test example', data.edit_content
  end
end
